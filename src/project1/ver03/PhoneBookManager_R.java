...
public class PhoneBookManager_R{
	
	public PhoneInfo[] pInfo;//폰인포 배열형식으로 pInfo 선언
	public int numOfpInfo;//한명 정보 추가시 1씩 증가
	
	//생성자-인자로 전달되는 num크기의 객체배열 생성
	public PhoneBookManager_R (int num) {
		pInfo = new PhoneInfo[num];//배열메모리를 잡아놓는다
		numOfpInfo = 0;
	}
	...
	public void dataSearch() {
		...
		for(int i=0; i<numOfpInfo;i++) {
			System.out.print("검색할 이름:"+pInfo[i].name);//이름으로 찾기
	...
	public void printMenu() {//메뉴출력 메소드를 폰매니저로 옮기고 메인에선 함수 호출
